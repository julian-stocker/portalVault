/**
 * Where a batch of navigation timings goes (ADR-0072).
 *
 ...
 * every failure is swallowed — `record_navigation()` refusing, the network
 * being gone, the session having expired. Telemetry that can break navigation
 * is worse than no telemetry, because it breaks the thing it exists to
 * measure.
 *
 * The account is not an argument. `record_navigation()` takes it from
 * `auth.uid()` and asks `has_tester_permission('performance_tracking')` before
 * it writes, so this file cannot record on anybody else's behalf however it is
 * called.
 */
"use server";

import { MAX_NAVIGATION_MS, type Sample } from "@/lib/perf/navigation";
...
/**
 * Everything wrong with a sample, or nothing.
 *
 ...
 */
function usable(sample: Sample): boolean {
  return (
...
export async function recordNavigations(batch: Batch): Promise<void> {
  try {
    ...
    for (const sample of batch.samples.slice(0, MAX_BATCH)) {
      ...
      /*
       * One call per navigation. A row-array RPC would be fewer round trips
